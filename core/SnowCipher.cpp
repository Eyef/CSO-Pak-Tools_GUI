...
// Sosemanuk finite-field lookup tables used by the LFSR feedback function.
static const uint32_t SosemanukMulAlpha[] = {
...
static const uint32_t SosemanukDivAlpha[] = {
...
// AES/Rijndael T-tables used by the cipher's FSM transform.
static const uint32_t RijndaelT0[] = {
...
static const uint32_t RijndaelT1[] = {
...
static const uint32_t RijndaelT2[] = {
...
static const uint32_t RijndaelT3[] = {
...
inline uint32_t SignExtendedByte(uint8_t value)
{
    if (value < 0x80)
        return value;
...
// Preserves the signed-char promotion behavior from the decompiled code.
// This is intentionally not a normal unsigned big-endian load.
inline uint32_t ReadLegacySignedKeyWord(const uint8_t* data)
{
    return SignExtendedByte(data[3]) |
           ((SignExtendedByte(data[2]) |
             ((SignExtendedByte(data[1]) |
               (SignExtendedByte(data[0]) << 8)) << 8)) << 8);
}

inline uint8_t ByteAt(uint32_t value, std::size_t bytePos)
{
...
{
    data[0] = ByteAt(value, 0);
    data[1] = ByteAt(value, 1);
    data[2] = ByteAt(value, 2);
    data[3] = ByteAt(value, 3);
}

inline uint32_t RijndaelTransform(uint32_t word)
{
    // One table lookup per byte reproduces the decompiled Rijndael round.
    return RijndaelT0[ByteAt(word, 0)] ^
           RijndaelT1[ByteAt(word, 1)] ^
           RijndaelT2[ByteAt(word, 2)] ^
           RijndaelT3[ByteAt(word, 3)];
}

inline uint32_t SosemanukLfsrFeedback(uint32_t xorWord, uint32_t mulWord,
                                      uint32_t divWord)
{
    // Shifted state words plus alpha tables form the next LFSR feedback word.
    return xorWord ^ (mulWord << 8) ^ (divWord >> 8) ^
           SosemanukDivAlpha[ByteAt(divWord, 0)] ^
           SosemanukMulAlpha[ByteAt(mulWord, 3)];
}

// Describes one unrolled step of the 16-word LFSR/FSM schedule.
struct KeyStreamStep
{
    uint8_t targetState;
    uint8_t xorState;
    uint8_t divState;
    uint8_t fsmAddState;
    uint8_t outputXorState;
};

constexpr std::array<KeyStreamStep, SNOW_NUM_BLOCKS> KeyStreamSchedule = {{
    { 15, 13, 4, 10, 14 },
    { 14, 12, 3, 9, 13 },
    { 13, 11, 2, 8, 12 },
    { 12, 10, 1, 7, 11 },
...
constexpr uint32_t KeyScheduleWarmupRounds = 2;

void SnowCipher::InitializeStateFromKey(const uint8_t* key)
{
    // The first four words keep legacy signed-byte expansion from the original.
    this->m_State[0] = ReadLegacySignedKeyWord(key);
    this->m_State[1] = ReadLegacySignedKeyWord(key + 4);
    this->m_State[2] = ReadLegacySignedKeyWord(key + 8);
    this->m_State[3] = ReadLegacySignedKeyWord(key + 12);

    for (std::size_t i = 0; i < 4; ++i)
        this->m_State[i + 4] = ~this->m_State[i];

    // The 16-word LFSR starts as key, bitwise inverse key, then a duplicate.
    for (std::size_t i = 0; i < 8; ++i)
        this->m_State[i + 8] = this->m_State[i];
}

void SnowCipher::SetKey(const uint8_t* key)
{
    this->InitializeStateFromKey(key);

    uint32_t fsmInput = 0;
    uint32_t fsmOutput = 0;
    uint32_t previousMixedLfsr = this->m_State[0];
    uint32_t lastFsmContribution = 0;

    // The key schedule runs two warm-up passes. Instead of emitting the FSM
    // result as keystream, each step folds it back into the next LFSR word.
    for (uint32_t round = 0; round < KeyScheduleWarmupRounds; ++round)
    {
        for (const KeyStreamStep &step : KeyStreamSchedule)
        {
            const uint32_t rawFeedback = SosemanukLfsrFeedback(
                this->m_State[step.xorState],
                this->m_State[step.targetState],
                this->m_State[step.divState]);
            lastFsmContribution = fsmOutput ^ (fsmInput + previousMixedLfsr);

            const uint32_t mixedLfsr = rawFeedback ^ lastFsmContribution;
            const uint32_t nextFsmOutput = RijndaelTransform(fsmInput);
            const uint32_t nextFsmInput =
                fsmOutput + this->m_State[step.fsmAddState];

            this->m_State[step.targetState] = mixedLfsr;
            previousMixedLfsr = mixedLfsr;
            fsmInput = nextFsmInput;
            fsmOutput = nextFsmOutput;
        }
    }

    this->m_State[16] = lastFsmContribution;
    this->m_State[17] = fsmInput;
    this->m_State[18] = fsmInput;
    this->m_State[19] = fsmOutput;

    this->m_BlocksAvailable = SNOW_NUM_BLOCKS;
}

void SnowCipher::GenerateKeyStreamBlock()
{
    // Produces 16 keystream words while rotating the LFSR state in-place.
    uint32_t fsmA = this->m_State[18];
    uint32_t fsmB = this->m_State[19];

    for (std::size_t outputIndex = 0; outputIndex < SNOW_NUM_BLOCKS; ++outputIndex)
    {
        const KeyStreamStep &step = KeyStreamSchedule[outputIndex];
        const uint32_t updatedLfsr = SosemanukLfsrFeedback(
            this->m_State[step.xorState],
            this->m_State[step.targetState],
            this->m_State[step.divState]);

        const uint32_t oldFsmA = fsmA;
        fsmA = this->m_State[step.fsmAddState] + fsmB;
        fsmB = RijndaelTransform(oldFsmA);

        // The same schedule updates state and chooses the word mixed into output.
        this->m_State[step.targetState] = updatedLfsr;
        this->m_State[17] = fsmA;
        this->m_State[18] = fsmA;
        this->m_State[19] = fsmB;
        this->m_Buffer[outputIndex] = this->m_State[step.outputXorState] ^
            fsmB ^ (fsmA + updatedLfsr);
    }
}

void SnowCipher::ProcessBufferImpl(uint8_t* outBuffer, const uint8_t* inBuffer,
                                   uint32_t dataSize, BufferMode mode)
{
    // SnowCipher operates on 32-bit words; callers enforce aligned lengths.
    uint32_t numBlocks = dataSize / SNOW_BLOCKS_SIZE;

    for (; numBlocks > 0; numBlocks--)
    {
        if (this->m_BlocksAvailable == SNOW_NUM_BLOCKS)
        {
            this->GenerateKeyStreamBlock();
            this->m_BlocksAvailable = 0;
        }

        const uint32_t sourceData = ReadU32Le(inBuffer);
        const uint32_t keyStreamWord = this->m_Buffer[this->m_BlocksAvailable];

        // This CSO variant decrypts by subtracting the stream word and encrypts by adding it.
        if (mode == BufferMode::Encrypt)
            WriteU32Le(outBuffer, sourceData + keyStreamWord);
        else
            WriteU32Le(outBuffer, sourceData - keyStreamWord);

        inBuffer += SNOW_BLOCKS_SIZE;
        outBuffer += SNOW_BLOCKS_SIZE;
        this->m_BlocksAvailable++;
    }
}

void SnowCipher::DecryptBufferImpl(uint8_t* outBuffer, const uint8_t* inBuffer,
                                   uint32_t dataSize)
{
    this->ProcessBufferImpl(outBuffer, inBuffer, dataSize, BufferMode::Decrypt);
}

void SnowCipher::EncryptBufferImpl(uint8_t* outBuffer, const uint8_t* inBuffer,
                                   uint32_t dataSize)
{
    this->ProcessBufferImpl(outBuffer, inBuffer, dataSize, BufferMode::Encrypt);
}
